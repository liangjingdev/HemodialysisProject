...
import static com.liangjing.hemodialysisproject.R.id.userCellPhone;

public class ChangeCellPhoneActivity extends ToolBarActivity implements View.OnClickListener {
    ...
    protected void setUpView() {
        etUserPhone = $(userCellPhone);
    }

    @Override
    protected void initEvents() {
        //设置数据
        etUserPhone.setText(bundle.getString("userCellPhone"));
        //将光标移动到尾部
        etUserPhone.setSelection(bundle.getString("userCellPhone") != null ? bundle.getString("userCellPhone").length() : 0);
        ...
        etUserPhone.addTextChangedListener(new TextWatcher() {
            ...
            public void onTextChanged(CharSequence s, int start, int before, int count) {
                ...
                if (etUserPhone.getText().toString().trim().length() > 0) {
    ...
    public void onClick(View v) {
        switch (v.getId()) {
            case R.id.save:
                String userCellPhone = etUserPhone.getText().toString().trim();

                //修改该用户在数据库中的用户名
                UserEntityHelper helper = DbUtil.getUserEntityHelper();
                UserEntity entity = helper.query((long) 1);
                entity.setCellPhone(userCellPhone);
                //更新该用户的数据到数据库中
