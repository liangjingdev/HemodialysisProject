...
import static com.liangjing.hemodialysisproject.R.id.userLocation;

public class ChangeUserLocationActivity extends ToolBarActivity implements View.OnClickListener {
    ...
    protected void setUpView() {
        etUserLocation = $(userLocation);
    }

    @Override
    protected void initEvents() {
        //设置数据
        etUserLocation.setText(bundle.getString("userLocation"));
        //将光标移动到尾部
        etUserLocation.setSelection(bundle.getString("userLocation") != null ? bundle.getString("userLocation").length() : 0);

        //保存按钮监听
        saveButton.setOnClickListener(this);

        //设置EditText监听
        etUserLocation.addTextChangedListener(new TextWatcher() {
            @Override
            ...
            public void onTextChanged(CharSequence s, int start, int before, int count) {
                ...
                if (etUserLocation.getText().toString().trim().length() > 0) {
